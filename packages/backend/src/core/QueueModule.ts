...
import {
	...
	SystemWebhookDeliverJobData, PostNoteCreatedJobData,
} from '../queue/types.js';
...
export type PostNoteCreatedQueue = Bull.Queue<PostNoteCreatedJobData>;
...
const $postNoteCreated: Provider = {
	provide: 'queue:postNoteCreated',
	useFactory: (config: Config) => new Bull.Queue(QUEUE.POST_NOTE_CREATED, baseQueueOptions(config, QUEUE.POST_NOTE_CREATED)),
	inject: [DI.config],
};
...
export class QueueModule implements OnApplicationShutdown {
	...
	public async dispose(): Promise<void> {
		...
		await Promise.all([
			...
			this.postNoteCreatedQueue.close(),
		]);
	}
